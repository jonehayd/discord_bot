...
export default {
  ...
  async execute(interaction) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(
          `blackjack_hit_${interaction.guildId}_${interaction.user.id}`,
        )
        .setLabel("Hit")
      ...
      new ButtonBuilder()
        .setCustomId(
          `blackjack_stay_${interaction.guildId}_${interaction.user.id}`,
        )
        .setLabel("Stay")
...
function createGameEmbed(game, isGameOver = false, won = false) {
  ...
  embed.addFields(
    {
      name: "🧑 Player Hand",
      value: `${playerCards} (Sum: ${playerSum})`,
      inline: false,
    },
    {
      name: "🤖 Bot Hand",
      value: `${botCards} (Sum: ${botSum})`,
      inline: false,
    },
  );
