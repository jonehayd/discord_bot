...
import hangmanManager from "../commands/games/hangman.js";
...
export default {
  ...
  async execute(interaction) {
    if (interaction.isButton() && interaction.customId.startsWith("hangman_")) {
      ...
      if (interaction.user.id !== ownerId) {
        return interaction.reply({
          content:
            "❌ This isn’t your game! Start your own with `/games hangman`.",
      ...
      if (type === "guess") {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Game ended by ${interaction.user}.\nThe word was: **${game.word}**`,
          )
          .setColor(0xff9900);
    ...
    if (
      ...
      interaction.customId.startsWith("hangman_modal_")
    ) {
      ...
      const guess = interaction.fields
        .getTextInputValue("letter_input")
        .toLowerCase();
      ...
      const embed = hangmanManager.createGameEmbed(
        game,
        result.gameOver,
        result.won,
      );
